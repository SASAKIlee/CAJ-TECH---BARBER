...
export default function Index() {
  ...
  const handleGerarPix = useCallback(async () => {
    ...
    try {
      ...
      const { data: barbeariaData } = await supabase.from("barbearias").select("id").eq("dono_id", user.id).single();
      ...
      const { data: fnData, error: fnError } = await supabase.functions.invoke("mercado-pago-pix", {
        body: { barbearia_id: barbeariaData.id, plano, email_dono: user.email || "financeiro@cajtech.net.br" },
      });
