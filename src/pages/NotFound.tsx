...
const NotFound = () => {
  ...
  return (
    <div className="dark min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden font-sans text-white">
      ...
      <div
        className="relative z-10 text-center space-y-6 max-w-md w-full p-8 bg-zinc-900/40 border border-zinc-800 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl"
        role="alert"
        aria-live="assertive"
      >
        <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-zinc-800/50 border border-zinc-700/50 shadow-inner mb-2">
          <AlertCircle className="h-10 w-10 text-emerald-500" aria-hidden="true" />
        </div>
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-zinc-400 font-medium leading-relaxed px-2">
            Parece que você se perdeu. O endereço{" "}
            <span className="text-emerald-500 font-mono text-[10px] bg-emerald-500/10 px-1.5 py-0.5 rounded break-all">
              {location.pathname}
            </span>{" "}
            não existe na CAJ TECH.
          </p>
        </div>

        <Button
          asChild
          className="w-full h-14 bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-lg transition-all active:scale-95 mt-2"
        >
