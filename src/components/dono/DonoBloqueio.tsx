...
export function DonoBloqueio({
  ...
  const valorFormatado = useMemo(
    () => formatarMoedaBR(getValorPlano(planoAtual)),
    [getValorPlano, planoAtual]
  );
    ...
    return (
      ...
      >
        <div className="h-28 w-28 bg-red-500/10 border border-red-500/20 rounded-full flex items-center justify-center mb-6 shadow-2xl">
          <Lock className="h-12 w-12 text-red-500" aria-hidden="true" />
        </div>
        <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter mb-3">
          Acesso Suspenso
        </h2>
        ...
        <Button
          onClick={() =>
            window.open("https://wa.me/5517992051576", "_blank", "noopener,noreferrer")
          }
          className="bg-zinc-800 h-14 px-8 rounded-2xl font-black uppercase tracking-widest"
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[85vh] px-4 py-10">
      <div
        className="bg-zinc-900 border border-red-500/40 p-8 rounded-[40px] max-w-md w-full text-center space-y-6 shadow-[0_0_80px_rgba(239,68,68,0.15)] relative overflow-hidden"
        role="alert"
        aria-live="assertive"
      >
        <div className="absolute top-0 left-0 w-full h-1 bg-red-500" aria-hidden="true" />
        <Lock className="w-16 h-16 text-red-500 mx-auto opacity-20 absolute -top-4 -right-4 rotate-12" aria-hidden="true" />
        <div className="space-y-2 relative z-10">
          ...
          <p className="text-zinc-400 text-sm font-medium">
            Renove sua assinatura para religar sua agenda online imediatamente.
          </p>
        </div>
