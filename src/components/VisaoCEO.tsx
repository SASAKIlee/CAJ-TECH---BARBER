...
import { VALORES_PLANO, getValorPlano } from "@/config/planos.config";
...
// VALORES_PLANO importado de @/config/planos.config
...
export function VisaoCEO({ totalLojas: _totalLojas = 0, vendedores = [] }: VisaoCEOProps) {
  ...
  // FUNÇÕES AUXILIARES
  // ==========================================
  // getValorPlano importada de @/config/planos.config — disponível diretamente
  const formatarMoeda = useCallback((v: number) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }), []);
  const formatarData = useCallback((d?: string) => d ? new Date(d).toLocaleDateString('pt-BR') : '---', []);
    ...
    try {
        ...
        leadsDoVendedor.forEach(lead => {
          ...
          if (lead.status === 'convertido') {
            ...
            if (!lead.dados_adicionais?.comissao_paga) {
              ...
              else taxa = Number(v.comissao_pro || 30);
              comissaoPendente += (valorPlano * taxa) / 100;
            }
          }
        });

        return { ...v, ativo: v.ativo !== false, leads_visita: qtdVisita, leads_pendente: qtdPendente, leads_convertido: qtdConvertido, mrr_gerado: mrrGerado, comissao_pendente: comissaoPendente };
      });
      ...
      if (carteiraDB) {
        const carteiraFormatada: CarteiraComissao[] = carteiraDB.map((c: any) => ({
          id: c.id, vendedor_id: c.vendedor_id, mes_referencia: c.mes_referencia,
          valor_acumulado: Number(c.valor_acumulado || 0), status: c.status,
          data_fechamento: c.data_fechamento, data_pagamento: c.data_pagamento,
      ...
      const totalReceita = lojasData.reduce((acc, l) => acc + (l.ativo !== false ? getValorPlano(l.plano) : 0), 0);
  ...
  const taxaConversao = useMemo(() => leadsRecentes.length > 0 ? ((statConvertidos / leadsRecentes.length) * 100).toFixed(0) : '0', [leadsRecentes.length, statConvertidos]);
  ...
  const handleAprovarContrato = useCallback(async (lead: Lead) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('aprovar-contrato-admin', {
        body: {
          lead_id: lead.id,
          nome_barbearia: lead.nome_barbearia,
          slug: slugDesejado,
      ...
      if (error || data?.error) throw new Error(error?.message || data?.error || "Erro no servidor ao aprovar.");
      ...
      if (extras.telefone) {
        const msg = `Fala mestre! Seu app CAJ TECH está no ar.\n🔗 cajtech.net.br/agendar/${slugDesejado}\n\nAcesse com o e-mail cadastrado: ${extras.email_dono || 'seu e-mail'}\n\nUse a senha que você escolheu no cadastro. Em caso de dúvida, fale com o suporte.`;
  ...
  const renovarLojaManual = useCallback(async (lojaId: string, dataAtual?: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('renovar-assinatura-admin', {
        body: { loja_id: lojaId, data_atual: dataAtual }
      });

      if (error || data?.error) throw new Error(error?.message || data?.error || "Erro ao renovar.");
  ...
  const excluirVendedor = useCallback(async (vendedorId: string) => {
    ...
    try {
      const { count, error: countError } = await supabase.from('leads').select('*', { count: 'exact', head: true }).eq('vendedor_id', vendedorId);
  ...
  const pagarComissao = useCallback(async (vendedorId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/pagar-comissao`, {
        ...
        body: JSON.stringify({ vendedor_id: vendedorId, valor: vendedor.comissao_pendente, vendedor_email: vendedor.email })
      });
  ...
  const handleCadastrarConsultor = useCallback(async () => {
    if (!novoConsultor.nome || !novoConsultor.email || !novoConsultor.senha) return toast.error("Preencha todos os campos!");
    setIsSavingConsultor(true);
    const toastId = toast.loading("Registrando consultor...");
    try {
      const { data, error } = await supabase.functions.invoke('criar-consultor-admin', {
        body: novoConsultor
      });

      if (error || data?.error) throw new Error(error?.message || data?.error || "Erro ao criar consultor.");
  ...
  const salvarComissao = useCallback(async () => {
    ...
    if ([valStarter, valPro, valElite].some(v => isNaN(v) || v < 0)) return toast.error("Valores inválidos.");
    try {
  ...
  const impersonarLoja = useCallback(async (loja: Loja) => {
    ...
    try {
      ...
      if (error || data?.error) throw new Error(error?.message || data?.error || "Erro ao gerar acesso de administrador.");

      toast.success("Acesso concedido!", { id: toastId });
      setTimeout(() => { if (isMountedRef.current) window.location.href = `/auth/callback?token=${data.token}`; }, 800);
    } catch (err: unknown) {
      toast.error("Falha ao entrar no painel.", { id: toastId });
    }
  }, []);
    ...
    <div className="space-y-6 pb-32 w-full max-w-full overflow-x-hidden p-4 bg-black min-h-screen text-white font-sans">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={tabAtiva} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <Card className="p-5 bg-zinc-900/40 border-zinc-800">
                  ...
                  {dataPlanos.length === 0 ? <p className="text-xs text-center text-zinc-600 italic py-10">Sem assinaturas ativas</p> : (
                    <div className="h-48 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={dataPlanos} cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={5} dataKey="value" stroke="none">{dataPlanos.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><RechartsTooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#27272a', borderRadius: '12px' }} /></PieChart></ResponsiveContainer></div>
                  )}
                </Card>
                <Card className="p-5 bg-zinc-900/40 border-zinc-800">
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-4 text-center">Saúde do Funil</p>
                  <div className="h-48 w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={dataFunil} margin={{ left: -20, bottom: 0 }}><XAxis dataKey="name" stroke="#777" fontSize={10} axisLine={false} tickLine={false} /><YAxis hide /><RechartsTooltip cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#27272a', borderRadius: '12px' }} /><Bar dataKey="value" radius={[6, 6, 6, 6]} maxBarSize={40}>{dataFunil.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}</Bar></BarChart></ResponsiveContainer></div>
                </Card>
              </div>

              <Card className="p-4 bg-blue-500/10 border-blue-500/20 space-y-3">
                <div className="flex items-center gap-2 text-blue-400"><Megaphone className="h-4 w-4" /><h3 className="font-black uppercase text-xs tracking-widest">Aviso para toda a rede</h3></div>
                <div className="flex gap-2">
                  ...
                  <Button onClick={dispararAviso} disabled={enviandoAviso} className="bg-blue-600 hover:bg-blue-500 font-bold text-xs text-white">{enviandoAviso ? "Enviando..." : "Enviar"}</Button>
                </div>
              </Card>
            </div>
          )}
                  ...
                  <Card key={lead.id} className="bg-zinc-900/50 border-zinc-800 overflow-hidden">
                    <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-zinc-800/30" onClick={() => setExpandido(expandido === lead.id ? null : lead.id)}>
                      <div>
                        ...
                        <p className="text-[9px] text-zinc-400 font-bold uppercase mt-1">👤 Consultor: <span className="text-emerald-400">{ranking.find(v => v.id === lead.vendedor_id)?.nome || "Desconhecido"}</span></p>
                      </div>
                    ...
                    {expandido === lead.id && (
                      <div className="p-4 bg-black/40 border-t border-zinc-800 space-y-5">
                        <div className="grid grid-cols-2 gap-3 text-[10px] font-black uppercase text-zinc-400 bg-black p-4 rounded-2xl border border-zinc-800/50">
                          ...
                          <div>Senha: <span className="text-zinc-500 block mt-0.5 italic text-[9px]">oculta por segurança</span></div>
                        ...
                        <div className="border border-zinc-800 rounded-2xl p-4 bg-zinc-900/30">
                          ...
                          <div className="rounded-xl p-4 flex flex-col gap-3 shadow-inner" style={{ backgroundColor: lead.dados_adicionais?.cor_secundaria || '#18181B' }}>
                            ...
                            <Button className="w-full font-black uppercase text-xs h-10 border-0" style={{ backgroundColor: lead.dados_adicionais?.cor_primaria || '#D4AF37', color: '#000' }}>Agendar Horário</Button>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                ...
                <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
                  {(['todas', 'ativas', 'trial', 'bloqueadas'] as FiltroLoja[]).map(f => (
                    <Button key={f} size="sm" variant={filtroLojas === f ? "default" : "outline"} onClick={() => setFiltroLojas(f)}
                      className={cn("rounded-xl h-8 text-[10px] uppercase font-black tracking-widest whitespace-nowrap", filtroLojas === f ? "bg-white text-black border-transparent" : "bg-black text-zinc-500 border-zinc-800")}>
                      {f === 'trial' ? "Em Teste" : f}
                    </Button>
                  ))}
                </div>
                  ...
                  <Card key={loja.id} className={cn("p-0 border overflow-hidden", loja.ativo === false ? "bg-red-500/5 border-red-500/20" : "bg-zinc-900/40 border-zinc-800")}>
                    <div className="p-4 cursor-pointer" onClick={() => setLojaExpandida(lojaExpandida === loja.id ? null : loja.id)}>
                      <div className="flex justify-between items-start">
                        ...
                        <span className={cn("px-2 py-0.5 rounded-md text-[9px] font-black uppercase border", loja.plano === 'elite' ? "bg-yellow-500/10 text-yellow-500 border-yellow-500/20" : "bg-zinc-800 text-zinc-400 border-zinc-700")}>{loja.plano || 'Pro'}</span>
                      </div>
                    ...
                    {lojaExpandida === loja.id && (
                      <div className="p-4 bg-black/40 border-t border-zinc-800/50 space-y-4">
                        ...
                        <Button onClick={() => renovarLojaManual(loja.id, loja.data_vencimento || undefined)} className="w-full bg-zinc-800 hover:bg-zinc-700 text-white font-black h-12 rounded-xl text-xs uppercase"><Calendar className="h-4 w-4 mr-2 text-emerald-500" /> Renovar Manualmente (+30 Dias)</Button>
                        <div className="flex gap-2 pt-2">
                          <Button variant="outline" onClick={() => toggleStatusLoja(loja.id, loja.ativo !== false)} className={cn("flex-1 h-12 text-[10px] font-black uppercase rounded-xl", loja.ativo === false ? "text-emerald-500 border-emerald-500/20 bg-emerald-500/5" : "text-red-500 border-red-500/20 bg-red-500/5")}>
                            {loja.ativo === false ? <Unlock className="h-4 w-4 mr-2" /> : <Lock className="h-4 w-4 mr-2" />}{loja.ativo === false ? "Religar Sistema" : "Bloquear Acesso"}
                          </Button>
                          <Button variant="outline" onClick={() => impersonarLoja(loja)} className="flex-1 h-12 text-[10px] font-black uppercase text-zinc-400 border-zinc-700 bg-black rounded-xl hover:bg-zinc-800"><Eye className="h-4 w-4 mr-2" /> Acessar Painel</Button>
                        </div>
                      </div>
                    )}
                  </Card>
          ...
          {tabAtiva === "equipe" && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-black uppercase italic text-white">Consultores</h2>
                  <p className="text-xs text-zinc-500 mt-1">{ranking.length} consultor(es) cadastrado(s)</p>
                </div>
                <Button onClick={() => setModalConsultorAberto(true)} className="bg-emerald-600 hover:bg-emerald-500 text-black font-bold text-xs uppercase h-10 rounded-xl gap-2"><UserPlus className="h-4 w-4" /> Novo Consultor</Button>
              </div>
              <div className="grid gap-3">
                {ranking.map(v => (
                  <Card key={v.id} className="bg-zinc-900/50 border-zinc-800 overflow-hidden">
                    <div className="p-4 cursor-pointer flex justify-between items-center" onClick={() => setVendedorExpandido(vendedorExpandido === v.id ? null : v.id)}>
                      <div className="flex items-center gap-3">
                        <div className={cn("h-2 w-2 rounded-full", v.ativo ? "bg-emerald-500" : "bg-red-500")} />
                        <div>
                          <h4 className="font-bold text-white text-sm uppercase italic">{v.nome}</h4>
                          <p className="text-[9px] text-zinc-500 font-bold">{v.email}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-emerald-500">{v.leads_convertido} fechados</span>
                        {vendedorExpandido === v.id ? <ChevronUp className="h-4 w-4 text-zinc-500" /> : <ChevronDown className="h-4 w-4 text-zinc-500" />}
                      </div>
                    </div>
                    {vendedorExpandido === v.id && (
                      <div className="p-4 bg-black/40 border-t border-zinc-800 space-y-3">
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div className="bg-zinc-900 rounded-xl p-3"><p className="text-[9px] text-zinc-500 font-black uppercase">Visitas</p><p className="text-lg font-black text-white">{v.leads_visita}</p></div>
                          <div className="bg-zinc-900 rounded-xl p-3"><p className="text-[9px] text-zinc-500 font-black uppercase">Pendentes</p><p className="text-lg font-black text-yellow-500">{v.leads_pendente}</p></div>
                          <div className="bg-zinc-900 rounded-xl p-3"><p className="text-[9px] text-zinc-500 font-black uppercase">MRR</p><p className="text-lg font-black text-emerald-500">{formatarMoeda(v.mrr_gerado)}</p></div>
                        </div>
                        <div className="flex items-center justify-between bg-zinc-900 rounded-xl p-3">
                          <div><p className="text-[9px] text-zinc-500 font-black uppercase">Comissão Pendente</p><p className="text-lg font-black text-yellow-500">{formatarMoeda(v.comissao_pendente)}</p></div>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => { setVendedorSelecionado(v); setComissaoStarter(String(v.comissao_starter || 20)); setComissaoPro(String(v.comissao_pro || 30)); setComissaoElite(String(v.comissao_elite || 40)); setModalComissaoAberto(true); }} className="h-8 text-[10px] border-zinc-700 text-zinc-400 rounded-xl"><Settings className="h-3 w-3 mr-1" /> Taxas</Button>
                            <Button size="sm" onClick={() => pagarComissao(v.id)} disabled={isPagarComissao || v.comissao_pendente <= 0} className="h-8 text-[10px] bg-emerald-600 hover:bg-emerald-500 text-black font-bold rounded-xl"><CreditCard className="h-3 w-3 mr-1" /> Pagar</Button>
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <Button variant="outline" onClick={() => toggleStatusVendedor(v.id, v.ativo)} className={cn("flex-1 h-10 text-[10px] font-black uppercase rounded-xl", v.ativo ? "text-red-500 border-red-500/20 bg-red-500/5" : "text-emerald-500 border-emerald-500/20 bg-emerald-500/5")}>{v.ativo ? "Desativar" : "Reativar"}</Button>
                          <Button variant="outline" onClick={() => excluirVendedor(v.id)} className="h-10 px-4 text-[10px] font-black uppercase text-red-500 border-red-500/20 bg-red-500/5 rounded-xl"><Trash2 className="h-3 w-3" /></Button>
                        </div>
                      </div>
                    )}
                  </Card>
                ))}
                {ranking.length === 0 && <p className="text-center text-xs font-bold text-zinc-600 uppercase italic py-10">Nenhum consultor cadastrado.</p>}
              </div>
            </div>
          )}
          ...
          {tabAtiva === "comissoes" && (
            <div className="space-y-4">
              <h2 className="text-lg font-black uppercase italic text-white">Carteira de Comissões</h2>
              {loadingCarteira ? (
                <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 text-zinc-500 animate-spin" /></div>
              ) : carteira.length === 0 ? (
                <p className="text-center text-xs font-bold text-zinc-600 uppercase italic py-10">Nenhuma comissão registrada.</p>
              ) : (
                <div className="grid gap-3">
                  {carteira.map(c => (
                    <Card key={c.id} className="bg-zinc-900/50 border-zinc-800 p-4 flex justify-between items-center">
                      <div>
                        <h4 className="font-bold text-white text-sm">{c.vendedor_nome}</h4>
                        <p className="text-[9px] text-zinc-500 font-bold uppercase">Ref: {c.mes_referencia}</p>
                      </div>
                      <div className="text-right">
                        <p className={cn("text-sm font-black", c.status === 'pago' ? "text-emerald-500" : "text-yellow-500")}>{formatarMoeda(c.valor_acumulado)}</p>
                        <span className={cn("text-[8px] font-black uppercase px-2 py-0.5 rounded-md border", c.status === 'pago' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-yellow-500/10 text-yellow-500 border-yellow-500/20")}>{c.status}</span>
                      </div>
                    </Card>
                  ))}
                </div>
      ...
      {modalConsultorAberto && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-md space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-black uppercase text-white">Novo Consultor</h3>
              <Button variant="ghost" size="icon" onClick={() => setModalConsultorAberto(false)} className="text-zinc-500"><X className="h-5 w-5" /></Button>
            </div>
            <Input placeholder="Nome completo" value={novoConsultor.nome} onChange={e => setNovoConsultor({ ...novoConsultor, nome: e.target.value })} className="bg-zinc-800 border-zinc-700 text-white h-12 rounded-xl" />
            <Input placeholder="E-mail" type="email" value={novoConsultor.email} onChange={e => setNovoConsultor({ ...novoConsultor, email: e.target.value })} className="bg-zinc-800 border-zinc-700 text-white h-12 rounded-xl" />
            <Input placeholder="Senha" type="password" value={novoConsultor.senha} onChange={e => setNovoConsultor({ ...novoConsultor, senha: e.target.value })} className="bg-zinc-800 border-zinc-700 text-white h-12 rounded-xl" />
            <div className="flex gap-2 pt-2">
      ...
      {/* ===== MODAL: EDITAR COMISSÕES ===== */}
      {modalComissaoAberto && vendedorSelecionado && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-md space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-black uppercase text-white">Taxas — {vendedorSelecionado.nome}</h3>
              <Button variant="ghost" size="icon" onClick={() => setModalComissaoAberto(false)} className="text-zinc-500"><X className="h-5 w-5" /></Button>
            </div>
            <div><label className="text-[10px] text-zinc-400 font-bold uppercase ml-1">Starter (%)</label><Input type="number" value={comissaoStarter} onChange={e => setComissaoStarter(e.target.value)} className="bg-zinc-800 border-zinc-700 text-white h-12 rounded-xl" /></div>
            <div><label className="text-[10px] text-zinc-400 font-bold uppercase ml-1">Pro (%)</label><Input type="number" value={comissaoPro} onChange={e => setComissaoPro(e.target.value)} className="bg-zinc-800 border-zinc-700 text-white h-12 rounded-xl" /></div>
            <div><label className="text-[10px] text-zinc-400 font-bold uppercase ml-1">Elite (%)</label><Input type="number" value={comissaoElite} onChange={e => setComissaoElite(e.target.value)} className="bg-zinc-800 border-zinc-700 text-white h-12 rounded-xl" /></div>
            <div className="flex gap-2 pt-2">
              <Button variant="outline" onClick={() => setModalComissaoAberto(false)} className="flex-1 border-zinc-700 text-zinc-400 h-12 rounded-xl">Cancelar</Button>
              <Button onClick={salvarComissao} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-black font-bold h-12 rounded-xl">Salvar</Button>
            </div>
          </div>
        </div>
      )}
