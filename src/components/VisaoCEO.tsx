...
export function VisaoCEO({ totalLojas: _totalLojas = 0, vendedores = [] }: VisaoCEOProps) {
  ...
  const impersonarLoja = useCallback(async (loja: Loja) => {
    try {
      ...
      if (authData?.user) {
        await supabase.from("audit_logs").insert({
          tabela: "barbearias", registro_id: loja.id, acao: "impersonate_enter",
          dados_novos: { target_slug: loja.slug, target_name: loja.nome }
        });
      }
    ...
    setTimeout(() => { if (isMountedRef.current) window.location.href = "/"; }, 800);
  }, []);
